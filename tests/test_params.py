...
# =============================================================================
# Phase 8 - report outputs
# =============================================================================
def test_the_report_section_exists_and_is_complete(params: dict[str, Any]) -> None:
    report = params["report"]
    for key in ("dpi", "figure_dir", "name_template", "width_inches", "decadal",
                "max_saturated_fraction", "stipple", "obs_count_vis",
                "facet_epochs", "facet_indices", "facet_max_points",
                "label_top_n", "cvd"):
        assert key in report, f"report.{key} is absent"


def test_report_figures_do_not_overwrite_the_phase_diagnostics(
    params: dict[str, Any],
) -> None:
    """``figures/`` holds the 150 dpi per-phase diagnostics; Phase 8 is separate.

    Writing both sets into one directory would leave a reader unable to tell
    which set a given PNG belongs to, and the Phase 8 run would silently replace
    diagnostics that can only be regenerated by re-running earlier notebooks in
    Colab.
    """
    assert params["report"]["figure_dir"] != "figures"
    assert params["report"]["dpi"] >= 300


def test_the_decadal_figure_pairs_a_pooled_row_with_a_single_sensor_row(
    params: dict[str, Any],
) -> None:
    """Figure 1's whole point is the contrast between the two rows.

    The Landsat row spans the L5/L7/L8 changeovers, whose measured offsets over
    Colombo are 2.4x and 3.4x the entire 26-year trend signal, so it is a
    SENSOR-STEP DIAGNOSTIC. The MODIS row is one sensor across all three
    windows. Setting both to the same source, or both to Landsat, would remove
    the only control the figure has.
    """
    decadal = params["report"]["decadal"]
    sources = {entry["key"]: entry for entry in params["uhi"]["suhii"]["sources"]}
    landsat = sources[decadal["landsat_source"]]
    modis = sources[decadal["modis_source"]]

    assert landsat["kind"] == "landsat"
    assert modis["kind"] == "modis"
    assert decadal["landsat_source"] != decadal["modis_source"]
    # A single-sensor-family Landsat source would defeat the diagnostic: the
    # top row has to be the POOLED series for the step to be visible at all.
    assert "sensors" not in landsat, (
        f"{decadal['landsat_source']} is restricted to one sensor family, so it "
        "cannot show the changeover step figure 1 exists to expose"
    )


def test_the_decadal_stretch_is_shared_and_ordered(params: dict[str, Any]) -> None:
    """One stretch across all six panels, or the step hides in the autoscaling."""
    stretch = params["report"]["decadal"]["shared_vis"]
    assert stretch["max"] > stretch["min"]
    # Land surface temperature over Colombo: a stretch that excludes the low
    # 20s or the low 40s would clip real pixels.
    assert stretch["min"] <= 26.0 and stretch["max"] >= 42.0


def test_the_decadal_palette_is_sequential_not_diverging(
    params: dict[str, Any],
) -> None:
    """Absolute temperature has no meaningful zero to centre a diverging ramp on.

    ``trends.slope_vis`` is diverging and must stay so - a slope has a real
    zero. This one must not be: a diverging ramp here would invent a neutral
    temperature and split the map either side of it.
    """
    from colombo_uhi import viz

    colours = params["report"]["decadal"]["palette"]
    profile = viz.palette_lightness_profile(colours)
    assert viz.is_monotonic(profile)
    assert not viz.is_diverging_monotonic(profile)


def test_the_facet_epochs_are_real_utfvi_epochs(params: dict[str, Any]) -> None:
    for epoch in params["report"]["facet_epochs"]:
        assert epoch in params["uhi"]["utfvi"]["epochs"], (
            f"report.facet_epochs names '{epoch}', which uhi.utfvi.epochs does "
            "not define"
        )


def test_the_facet_indices_are_computable(params: dict[str, Any]) -> None:
    for name in params["report"]["facet_indices"]:
        assert name in params["uhi"]["drivers"]["predictors"], (
            f"report.facet_indices names '{name}', which the driver stack does "
            "not produce"
        )


def test_the_stipple_lattice_is_sparse_enough_to_see_through(
    params: dict[str, Any],
) -> None:
    """At stride 1 a significant region becomes a solid mask.

    That hides the slope underneath it, which defeats an overlay whose whole
    job is to sit on top of a readable map.
    """
    assert int(params["report"]["stipple"]["stride"]) >= 2


def test_labelling_sixty_divisions_is_not_attempted(params: dict[str, Any]) -> None:
    """Sixty labels over the CMC core is an unreadable mat.

    The exported priority set is ``greening.top_n`` (60); the MAP labels far
    fewer, and the full list travels as a table instead.
    """
    assert 1 <= params["report"]["label_top_n"] <= 20
    assert params["report"]["label_top_n"] < params["greening"]["top_n"]


def test_every_cvd_entry_names_a_resolvable_palette_and_a_known_test(
    params: dict[str, Any],
) -> None:
    from colombo_uhi import viz
    ...
    for entry in params["report"]["cvd"]["palettes"]:
        ...
        colours = viz.resolve_palette(params, entry["path"])
        assert len(colours) >= 2, f"{entry['path']} has fewer than two colours"


def test_every_exempt_palette_states_why(params: dict[str, Any]) -> None:
    """An exemption without a reason is a way to make the check pass silently."""
    for entry in params["report"]["cvd"]["palettes"]:
        if entry.get("exempt"):
            assert entry.get("reason", "").strip(), (
                f"{entry['path']} is exempt from the colour-vision check with no "
                "stated reason"
            )


def test_the_three_simulated_deficiencies_are_the_dichromacies(
    params: dict[str, Any],
) -> None:
    assert set(params["report"]["cvd"]["deficiencies"]) == {
        "deuteranopia", "protanopia", "tritanopia"
    }


def test_the_two_new_caveats_are_defined(params: dict[str, Any]) -> None:
    for key in ("colour_is_not_the_only_channel", "figures_are_derived_not_authored"):
        assert key in params["caveats"], f"caveats.{key} is absent"
        assert len(params["caveats"][key].split()) > 20, (
            f"caveats.{key} is too short to say anything"
        )
